...
\author{Rayan Singh}
\email{rayans2@illinois.edu}
\affiliation{\institution{University of Illinois Urbana-Champaign}\country{}}
\author{Ryan Vir}
\email{ryansv2@illinois.edu}
\affiliation{\institution{University of Illinois Urbana-Champaign}\country{}}
\author{Tony Chan}
\email{tchan34@illinois.edu}
\affiliation{\institution{University of Illinois Urbana-Champaign}\country{}}

\begin{abstract}
FilmSnob is a content-based movie recommender. Given films a user has rated on Letterboxd, it builds a composite embedding for each film using SBERT to encode plot descriptions and user reviews, combined with structured metadata like genre, year, and popularity. PCA reduces these to 128 dimensions. The system then constructs a rating-weighted user profile vector and ranks unseen films by cosine similarity.

Data comes from two sources. The first is the Letterboxd platform, which provides user ratings and written reviews via public RSS feeds. The second is the TMDB API, which supplies plot summaries, genres, cast, director, keywords, and community ratings for each film.

We evaluate the system qualitatively by generating recommendations for real Letterboxd users and checking whether the results align with their taste. Quantitative evaluation using train/test splits and metrics like Recall@10 is planned for the final report.

The first few weeks of the project were spent researching existing methods in sentence embeddings and recommendation systems. The following weeks were for building the scraping and embedding pipeline and writing the first milestone report. The final weeks are for formal evaluation, baseline comparisons, and the final report.
\end{abstract}
...
Letterboxd hosts millions of user-generated film ratings and written reviews. These capture two distinct signals of user preference: numerical ratings, and natural-language reviews that often convey nuance a star count cannot. Despite this, the platform does not offer personalized recommendations based on a user's own data.

FilmSnob addresses this. Given a user's Letterboxd history, the system encodes each film's content and user reviews into dense vectors using SBERT, combines those with structured metadata (genre, year, director, popularity), and builds a rating-weighted preference profile. It then ranks unseen films by cosine similarity to that profile and returns the top matches.

The central challenge is turning messy, non-standardized data into something comparable. A five-star rating compresses a user's opinion into a single number. A written review is richer but harder to work with. We encode both into a shared embedding space using SBERT, add structured features, and reduce the result with PCA to get a compact vector per film that captures content, reception, and metadata.
...
\subsubsection{Letterboxd Ingestion}
...
\subsubsection{Cross-User Aggregation}
...
\subsubsection{Content Embedding (384 dimensions)}

We concatenate the plot overview, genre names, director, and keywords into one string (\texttt{embedding\_text}) and run it through \texttt{all-MiniLM-L6-v2} (22M params, 384-d output). This gives us a 384-dimensional vector for each film's content.

\subsubsection{Review Embedding (384 dimensions)}
...
\subsubsection{Scalar Features (4 dimensions)}
...
\subsubsection{Genre Multi-Hot (19 dimensions)}
...
All code is available at \url{https://github.com/rayansingh/FilmSnob}. To run the full pipeline from scratch:
...
For our initial evaluation, we plan to do a train/test split on individual users' Letterboxd rating histories, using around 50\% of their ratings for training and the remaining 50\% for testing. The predictive model will be evaluated by comparing predicted ratings against actual user ratings using standard regression metrics like mean absolute error (MAE) and root mean squared error (RMSE). For the retrieval component, we will check whether top-ranked recommendations align with the user's known preferences in genre and theme. Early results should show whether the hybrid approach performs better than a simple content-based or predictive ranking method, specifically for users with a moderate rating history.

As a qualitative check, we picked three Letterboxd users with different taste to see how the system handles each one. Table~\ref{tab:recs} lists their highest-rated films next to what our system recommended. Because we pull data from each user's RSS feed, which Letterboxd caps at 50 entries, these results are based on their 50 most recent diary entries rather than their full watch history (which may be much larger). Recommendations take under two seconds per user.
...
With semantic embedding, we can use extracted data from user reviews to add more meaningful signals when ratings are ambiguous or inconsistent. We also observed that user preferences are often more complex than simple genre matching; factors like themes captured in text embeddings play a major role in improving recommendations. Current limitations include some sarcastic reviews that are inconsistent with the user's rating, and balancing the weight between numerical ratings and textual sentiment, which will be a focus for fine-tuning the model.
